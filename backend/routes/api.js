...
router.get("/top-hashtags", async (req, res) => {
  ...
  try {
    ...
    const top_list_hashtag = await db
      .collection("tweets")
      .aggregate(pipeline)
      .toArray();

    res.json(
      top_list_hashtag.map((tag) => ({
        hashtag: tag._id,
        count: tag.count,
      }))
    );
    ...
    res
      .status(500)
      .json({ error: "Internal server error (hashtag analysis failed)" });
  }
});
